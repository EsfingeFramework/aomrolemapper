...
public class GenerateClassesUsingAdapterImprovedV2 {
		...
		if (storedClasses.containsKey(entity.getEntityType().getName())) {
			...
			for (IProperty p : entity.getProperties()) {
				...
				if (p.getPropertyType() != null) {
					...
					// System.out.println(p.getPropertyType().getType().toString()
					// +
					// "****\n");
					// System.out.println(p.getPropertyType().getType().toString().substring(6));
				}
				...
				if (!p.getPropertyType().getType().toString().substring(6)
					...
					// createGetter(String classname, ClassWriter cw, String
					// prop)
